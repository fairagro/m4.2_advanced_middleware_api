...
Product stack, module graph, and scaling notes for the Advanced Middleware API live below. Surface-bar path rows for
...
The following technologies are foundational to the middleware API. Component specs may assume their presence and must
not replace them with alternatives without a project-level decision recorded here.
...
- Runtime configuration is read from YAML via `ConfigWrapper` (`middleware.shared.config.config_wrapper`).
- **No `os.environ` calls in application code.** Environment variables are resolved by `ConfigWrapper` only.
...
- ARC objects must not cross process boundaries via pickle — serialize to JSON first (they carry .NET interop state).
...
- Capability specs live under `openspec/specs/<domain>/` with kebab-case domain names that mirror the primary code
  artifact they describe (e.g. `ArcManager` → `openspec/specs/arc-manager/`).
