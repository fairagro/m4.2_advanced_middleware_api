...
| **2** | Finalize trigger and HTTP semantics? | Mark harvest **`COMPLETED` first**, then **enqueue** async Celery finalize. HTTP MUST NOT wait for Git push. On catalog push failure: harvest stays `COMPLETED`; record `CATALOG_PUSH_FAILED`; finalize MAY be retried without re-opening the harvest. |
