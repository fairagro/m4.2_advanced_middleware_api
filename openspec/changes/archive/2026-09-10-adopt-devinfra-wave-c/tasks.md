...
- [x] 2.3 Replace `docker/Dockerfile.api` with thin Alpine last stage (runtime apk + git config + HEALTHCHECK; COPY from
      `export_bins`)
...
- [x] 3.2 Update `feature-pull-request.yml` (keep detect-changes; CQ SHA-pin #72; build/release `@main`; check → local)
