...
class Config(ConfigBase):
    ...
    api_url: Annotated[
        str, Field(description="Base URL of the Middleware API (e.g., https://api.example.com)")
    ]
    client_cert_path: Annotated[
        str, Field(description="Path to the client certificate file in PEM format")
    ]
    client_key_path: Annotated[
        str, Field(description="Path to the client private key file in PEM format")
    ]
    ca_cert_path: Annotated[
        str | None, Field(description="Path to the CA certificate file for server verification (optional)")
    ] = None
    timeout: Annotated[
        float, Field(description="Request timeout in seconds", gt=0)
    ] = 30.0
    verify_ssl: Annotated[
        bool, Field(description="Enable SSL certificate verification")
    ] = True
