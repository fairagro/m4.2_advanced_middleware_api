...
def test_validate_url_scheme_valid() -> None:
    ...
    GitRepoConfig(url="https://example.com/repo.git", group="group", cache_dir=Path("/tmp"))
    GitRepoConfig(url="file:///tmp/repo.git", group="group", cache_dir=Path("/tmp"))
    GitRepoConfig(url="http://example.com/repo.git", group="group", cache_dir=Path("/tmp"))


def test_validate_url_scheme_invalid() -> None:
    """Test invalid URL schemes."""
    with pytest.raises(ValidationError) as excinfo:
        GitRepoConfig(url="ftp://example.com/repo.git", group="group", cache_dir=Path("/tmp"))
...
def test_check_health_file_scheme() -> None:
    ...
    config = GitRepoConfig(url="file:///non/existent/path", group="group", cache_dir=Path("/tmp"))
    repo = GitRepo(config)
...
def test_check_health_https_success(mock_urlopen: MagicMock) -> None:
    ...
    config = GitRepoConfig(url="https://example.com", group="group", cache_dir=Path("/tmp"))
    repo = GitRepo(config)
