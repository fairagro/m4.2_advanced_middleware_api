...
def test_health_check_success(client: TestClient) -> None:
    ...
    with unittest.mock.patch("middleware.api.api.redis.from_url", return_value=mock_redis), \
...
def test_health_check_failure(client: TestClient) -> None:
    ...
    # Mock Redis failure
    with unittest.mock.patch("middleware.api.api.redis.from_url", side_effect=redis.exceptions.RedisError("Redis down")), \
         unittest.mock.patch("middleware.api.api.celery_app.connection_or_acquire", side_effect=Exception("RabbitMQ down")):
