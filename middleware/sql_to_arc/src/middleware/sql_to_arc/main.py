...
async def fetch_studies(cur: psycopg.AsyncCursor[dict[str, Any]], investigation_id: int) -> list[dict[str, Any]]:
    ...
    await cur.execute(
        'SELECT id, investigation_id, title, description, submission_time, release_time FROM "ARC_Study" WHERE investigation_id = %s',
        (investigation_id,),
    )
    return await cur.fetchall()
...
async def process_batch(client: ApiClient, batch: list[ArcInvestigation]) -> None:
    ...
    try:
        ...
        logger.info(f"Batch upload successful. Created/Updated: {len(response.arcs)}")
